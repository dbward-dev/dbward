...
impl RejectRequest {
    pub fn execute(&self, input: RejectRequestInput, user: &AuthUser) -> Result<RejectRequestOutput, AppError> {
        // 0. Input validation
        ...
        let workflow: Option<Workflow> = request.workflow_snapshot_json.as_deref()
            .and_then(|json| serde_json::from_str(json).ok());
        let approvals = self.request_repo.get_approvals(&request.id)?;
        let current_step_index = workflow.as_ref()
            .map(|wf| dbward_domain::services::workflow_matcher::find_current_step(&wf.steps, &approvals))
            .unwrap_or(0);
        ...
        let matched_selector = if is_requester {
            ...
            if current_step_index < wf.steps.len() as u32 {
                ...
                wf.steps[current_step_index as usize].approvers.iter()
                    .find(|ag| ag.selector.matches(&role_names, &user.groups, &user.subject_id, false))
...
mod tests {
    ...
    struct FakeIdGen;
    impl IdGenerator for FakeIdGen { fn generate(&self) -> String { "rej-001".into() } }

    struct FakeRepo { request: Mutex<Option<Request>>, rejected: Mutex<bool> }
    ...
    fn make_pending_request() -> Request {
        let wf = Workflow {
            ...
            operations: vec![],
            steps: vec![WorkflowStep { approvers: vec![ApproverGroup { selector: Selector::Role("dba".into()), min: 1 }], mode: WorkflowStepMode::Any }],
        ...
        Request {
            ...
            operation: Operation::ExecuteDml, detail: "DELETE FROM users".into(),
            status: RequestStatus::Pending, emergency: false, reason: None,
            idempotency_key: None, metadata_json: "{}".into(), share_with: vec![],
            no_store: false, workflow_snapshot_json: Some(serde_json::to_string(&wf).unwrap()),
