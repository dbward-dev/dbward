...
pub async fn run_self_update() -> Result<CliResponse<UpdateOutput>, CliError> {
    ...
    let assets = release["assets"]
        .as_array()
        .ok_or_else(|| CliError::Server("no assets in release".into()))?;

    let asset_url = assets
        .iter()
        .find(|a| a["name"].as_str() == Some(&asset_name))
        .and_then(|a| a["browser_download_url"].as_str())
        .ok_or_else(|| CliError::Server(format!("asset {asset_name} not found in release")))?;
    ...
    let tmp_dir = tempfile::tempdir().map_err(|e| CliError::Server(e.to_string()))?;
    ...
    for entry in archive
        ...
        .map_err(|e| CliError::Server(e.to_string()))?
    {
        let mut entry = entry.map_err(|e| CliError::Server(e.to_string()))?;
        let path = entry.path().map_err(|e| CliError::Server(e.to_string()))?;
        ...
        entry
            ...
            .map_err(|e| CliError::Server(format!("failed to extract: {e}")))?;
    }
    ...
    let current_exe = std::env::current_exe().map_err(|e| CliError::Server(e.to_string()))?;
    let new_path = current_exe.with_extension("new");

    std::fs::copy(&extracted_binary, &new_path)
        .map_err(|e| CliError::Server(format!("failed to write new binary: {e}")))?;

    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        std::fs::set_permissions(&new_path, std::fs::Permissions::from_mode(0o755))
            .map_err(|e| CliError::Server(format!("failed to set permissions: {e}")))?;
    }

    std::fs::rename(&new_path, &current_exe)
        .map_err(|e| CliError::Server(format!("failed to replace binary: {e}")))?;
...
async fn check_latest() -> Result<String, CliError> {
    ...
    let resp: serde_json::Value = client
        ...
        .send()
        .await
        .map_err(|e| CliError::Server(format!("failed to check for updates: {e}")))?
        .json()
        .await
        .map_err(|e| CliError::Server(e.to_string()))?;
    ...
    let tag = resp["tag_name"]
        .as_str()
        .ok_or_else(|| CliError::Server("could not determine latest version".into()))?;
