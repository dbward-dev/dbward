...
impl dbward_app::ports::PreflightJobRepo for StubPreflightJobRepo {
    fn create_with_limit(&self, _: &dbward_app::ports::PreflightJob, _: u32) -> Result<(), AppError> { Ok(()) }
    fn claim_for_agent(&self, _: &str, _: &[(String, String)], _: usize) -> Result<Vec<dbward_app::ports::PreflightJob>, AppError> { Ok(vec![]) }
    ...
    fn get(&self, _: &str) -> Result<Option<dbward_app::ports::PreflightJob>, AppError> { Ok(None) }
    fn mark_expired(&self) -> Result<u64, AppError> { Ok(0) }
    fn purge_old(&self, _: u64) -> Result<u64, AppError> { Ok(0) }
}
...
fn test_state() -> AppState {
    AppStateBuilder {
        ...
        preflight_notifier: std::sync::Arc::new(dbward_server::preflight_notifier::PreflightNotifier::new()),
