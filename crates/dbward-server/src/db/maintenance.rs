...
pub fn collect_expired_results(conn: &Connection) -> Result<Vec<(String, String)>, rusqlite::Error> {
    let now = chrono::Utc::now().to_rfc3339();
...
pub fn delete_expired_result_records(conn: &Connection, request_id: &str) -> Result<(), rusqlite::Error> {
    conn.execute("DELETE FROM result_access WHERE request_id = ?1", rusqlite::params![request_id])?;
    conn.execute("DELETE FROM request_results WHERE request_id = ?1", rusqlite::params![request_id])?;
    Ok(())
}
