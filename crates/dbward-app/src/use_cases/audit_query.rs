...
mod tests {
    ...
    impl Authorizer for AllowViewOnly {
        fn authorize_global(&self, _: &AuthUser, perm: Permission) -> Result<(), AuthzError> {
            if perm == Permission::AuditView { Ok(()) }
            else { Err(AuthzError::Forbidden { permission: perm, reason: "denied".into() }) }
        }
        fn authorize_scoped(&self, _: &AuthUser, _: Permission, _: &DatabaseName, _: &Environment, _: &ResourceContext) -> Result<(), AuthzError> { Ok(()) }
    }
    struct DenyAll;
    impl Authorizer for DenyAll {
        fn authorize_global(&self, _: &AuthUser, perm: Permission) -> Result<(), AuthzError> {
            Err(AuthzError::Forbidden { permission: perm, reason: "denied".into() })
        }
        fn authorize_scoped(&self, _: &AuthUser, _: Permission, _: &DatabaseName, _: &Environment, _: &ResourceContext) -> Result<(), AuthzError> {
            Err(AuthzError::Forbidden { permission: Permission::AuditView, reason: "denied".into() })
        }
    }
    ...
    impl AuditRepo for FakeAuditRepo {
        fn list(&self, filter: &AuditFilter) -> Result<Vec<AuditEvent>, AppError> {
            ...
            if let Some(ref actor) = filter.actor_id {
                if actor != "alice" { return Ok(vec![]); }
            }
            Ok(vec![ev])
        }
        fn verify_chain(&self) -> Result<AuditVerifyResult, AppError> {
            Ok(AuditVerifyResult { total_events: 42, first_broken_id: None })
        }
        fn purge_old(&self, _: &str) -> Result<u32, AppError> { Ok(0) }
    }

    fn admin_user() -> AuthUser {
        AuthUser { subject_id: "admin".into(), subject_type: SubjectType::User, roles: vec![ResolvedRole { name: "admin".into(), permissions: [P::AuditViewAll].into_iter().collect(), databases: vec![], environments: vec![] }], groups: vec![], token_id: None }
    }
    fn normal_user() -> AuthUser {
        AuthUser { subject_id: "alice".into(), subject_type: SubjectType::User, roles: vec![], groups: vec![], token_id: None }
    }

    #[test]
    fn admin_sees_all_with_detail_raw() {
        let uc = AuditQuery { authorizer: Arc::new(AllowAll), audit_repo: Arc::new(FakeAuditRepo) };
        let out = uc.list(AuditListInput { filter: AuditFilter { actor_id: None, event_type: None, event_category: None, outcome: None, environment: None, database: None, since: None, until: None, limit: 100, offset: 0 } }, &admin_user()).unwrap();
    ...
    fn user_sees_own_events_redacted() {
        let uc = AuditQuery { authorizer: Arc::new(AllowViewOnly), audit_repo: Arc::new(FakeAuditRepo) };
        let out = uc.list(AuditListInput { filter: AuditFilter { actor_id: None, event_type: None, event_category: None, outcome: None, environment: None, database: None, since: None, until: None, limit: 100, offset: 0 } }, &normal_user()).unwrap();
        assert_eq!(out.events.len(), 1);
    ...
    fn unauthorized_returns_forbidden() {
        let uc = AuditQuery { authorizer: Arc::new(DenyAll), audit_repo: Arc::new(FakeAuditRepo) };
        assert!(matches!(uc.list(AuditListInput { filter: AuditFilter { actor_id: None, event_type: None, event_category: None, outcome: None, environment: None, database: None, since: None, until: None, limit: 100, offset: 0 } }, &normal_user()), Err(AppError::Forbidden(_))));
    }
