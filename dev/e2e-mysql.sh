...
ADMIN_TOKEN=$(docker compose exec -T dbward-server /app/dbward server token create --user admin1 --role admin --data /data 2>/dev/null | grep -o 'dbw_[a-z0-9]*')
DEV_TOKEN=$(docker compose exec -T dbward-server /app/dbward server token create --user dev1 --role developer --data /data 2>/dev/null | grep -o 'dbw_[a-z0-9]*')
