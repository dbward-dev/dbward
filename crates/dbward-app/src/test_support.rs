...
impl crate::ports::SchemaRepo for FakeSchemaRepo {
    fn upsert_snapshot(&self, _: &crate::ports::SchemaSnapshotRecord) -> Result<(), crate::error::AppError> {
        Ok(())
    }
    fn get_snapshot(&self, _: &str, _: &str) -> Result<Option<crate::ports::SchemaSnapshotRecord>, crate::error::AppError> {
        Ok(None)
    }
...
impl crate::ports::DryRunRepo for FakeDryRunRepo {
    fn create_jobs(&self, _: &[crate::ports::DryRunJobRecord]) -> Result<(), crate::error::AppError> {
        Ok(())
    }
    fn find_pending_for_agent(&self, _: &[(String, String)]) -> Result<Vec<crate::ports::DryRunJobRecord>, crate::error::AppError> {
        Ok(vec![])
    }
    fn claim(&self, _: &str, _: &str, _: &str, _: &str) -> Result<bool, crate::error::AppError> {
        Ok(false)
    }
    fn complete(&self, _: &str, _: &str, _: &str, _: &str, _: &str) -> Result<bool, crate::error::AppError> {
        Ok(false)
    }
    fn fail(&self, _: &str, _: &str, _: &str, _: &str, _: &str) -> Result<bool, crate::error::AppError> {
        Ok(false)
    }
    fn reclaim_stale(&self, _: &str) -> Result<u32, crate::error::AppError> {
        Ok(0)
    }
    fn find_for_request(&self, _: &str) -> Result<Vec<crate::ports::DryRunJobRecord>, crate::error::AppError> {
        Ok(vec![])
    }
}
