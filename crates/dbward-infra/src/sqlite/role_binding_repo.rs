...
impl RoleBindingRepo for SqliteRoleBindingRepo {
    ...
    fn create(
        ...
        let conn = self.conn.lock();
        let subjects_json =
            serde_json::to_string(subjects).map_err(|e| AppError::Internal(e.to_string()))?;
        let groups_json =
            serde_json::to_string(groups).map_err(|e| AppError::Internal(e.to_string()))?;
    ...
    fn list(&self) -> Result<Vec<dbward_app::ports::RoleBindingEntry>, AppError> {
        ...
        for row in rows {
            let (id, role, subj_json, grp_json) =
