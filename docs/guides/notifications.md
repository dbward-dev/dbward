...
Run `dbward-server validate --config server.toml` first — it validates webhook URL format and config. Add `--preflight` to also check Slack API connectivity (note: webhook URL reachability is not checked).
