...
mod tests {
    ...
    use dbward_domain::entities::{User, Token, Request, Approval, AuditEvent};
    ...
    impl Authorizer for DenyAll {
        fn authorize_global(&self, _: &AuthUser, _: Permission) -> Result<(), AuthzError> {
            Err(AuthzError::Forbidden { permission: P::UserManage, reason: "denied".into() })
        }
    ...
    struct FakeAudit;
    impl AuditLogger for FakeAudit { fn record(&self, _: &AuditEvent) -> Result<(), AppError> { Ok(()) } }

    struct FakeUserRepo { has_user: bool }
    impl UserRepo for FakeUserRepo {
        fn upsert(&self, _: &User) -> Result<(), AppError> { Ok(()) }
        fn get(&self, _: &str) -> Result<Option<User>, AppError> {
            if self.has_user { Ok(Some(User { id: "u1".into(), display_name: None, email: None, groups: vec![], roles: vec![], status: dbward_domain::entities::UserStatus::Active, last_seen_at: None, created_at: Utc::now(), updated_at: Utc::now() })) }
            else { Ok(None) }
        }
    ...
    impl TokenRepo for FakeTokenRepo {
        ...
        fn revoke(&self, _: &str, _: DateTime<Utc>) -> Result<bool, AppError> { Ok(true) }
        fn revoke_all_for_user(&self, _: &str, _: DateTime<Utc>) -> Result<u32, AppError> { Ok(2) }
    ...
    impl RequestRepo for FakeRequestRepo {
        ...
        fn cancel_all_for_user(&self, _: &str, _: DateTime<Utc>) -> Result<u32, AppError> { Ok(3) }
    ...
    fn admin_user() -> AuthUser {
        AuthUser {
            ...
            roles: vec![ResolvedRole { name: "admin".into(), permissions: [P::UserManage].into_iter().collect(), databases: vec![], environments: vec![] }],
            groups: vec![],
            token_id: None,
        }
    }
    ...
    fn make_uc(has_user: bool, authz: Arc<dyn Authorizer>) -> UserManage {
        UserManage { authorizer: authz, user_repo: Arc::new(FakeUserRepo { has_user }), token_repo: Arc::new(FakeTokenRepo), request_repo: Arc::new(FakeRequestRepo), audit: Arc::new(FakeAudit), clock: Arc::new(FakeClock) }
    }

    #[test]
    fn list_denied_without_permission() {
        let uc = make_uc(false, Arc::new(DenyAll));
        assert!(matches!(uc.list(&admin_user()), Err(AppError::Forbidden(_))));
    }

    #[test]
    fn suspend_not_found() {
        let uc = make_uc(false, Arc::new(AllowAll));
        assert!(matches!(uc.suspend(UserSuspendInput { user_id: "ghost".into() }, &admin_user()), Err(AppError::NotFound(_))));
    }

    #[test]
    fn suspend_success_returns_counts() {
        let uc = make_uc(true, Arc::new(AllowAll));
        let out = uc.suspend(UserSuspendInput { user_id: "u1".into() }, &admin_user()).unwrap();
    ...
    fn activate_not_found() {
        let uc = make_uc(false, Arc::new(AllowAll));
        assert!(matches!(uc.activate("ghost", &admin_user()), Err(AppError::NotFound(_))));
    }
}
