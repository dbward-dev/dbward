...
mod tests {
    ...
    impl FakeRequestRepo {
        fn with_status(status: RequestStatus) -> Self {
            Self { request: Mutex::new(Some(DomainRequest {
                id: "req-1".into(),
        ...
        fn empty() -> Self { Self { request: Mutex::new(None) } }
    }
    ...
    async fn not_found_returns_error() {
        let uc = StreamResult { authorizer: Arc::new(AllowAll), request_repo: Arc::new(FakeRequestRepo::empty()), result_channel: Arc::new(FakeResultChannel) };
        let result = uc.execute(StreamResultInput { request_id: "nope".into(), timeout_secs: None }, &user()).await;
        assert!(matches!(result, Err(AppError::NotFound(_))));
    }

    #[tokio::test]
    async fn terminal_state_returns_immediately() {
        let uc = StreamResult { authorizer: Arc::new(AllowAll), request_repo: Arc::new(FakeRequestRepo::with_status(RequestStatus::Executed)), result_channel: Arc::new(FakeResultChannel) };
        let out = uc.execute(StreamResultInput { request_id: "req-1".into(), timeout_secs: None }, &user()).await.unwrap();
    ...
    async fn pending_state_subscribes_and_returns_none_on_timeout() {
        let uc = StreamResult { authorizer: Arc::new(AllowAll), request_repo: Arc::new(FakeRequestRepo::with_status(RequestStatus::Pending)), result_channel: Arc::new(FakeResultChannel) };
        let out = uc.execute(StreamResultInput { request_id: "req-1".into(), timeout_secs: Some(1) }, &user()).await.unwrap();
